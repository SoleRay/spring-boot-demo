...
public class JwtUtil {
    ...
    /**
     *  token = base64(head).base64(body).sign
     *
     *  sign = hash(base64(head) + base64(body) + 密文)
     *
     *  token，由服务器生成然后下发给客户端
     *  客户端，每次请求发送token给服务器端，服务器端拆开token，进行sign的校验。
     *
     */
...
//    }
    ...
    public static void main(String[] args) {
        byte[] head = Base64.getDecoder().decode("eyJhbGciOiJIUzUxMiJ9");
        System.out.println(new String(head));

		byte[] body = Base64.getDecoder().decode("eyJzdWIiOiJ1c2VyaWQ9MSxyb2xlPWFkbWluLHByaWNlPTM5OCIsImlhdCI6MTU5MDQxMzk3NywiZXhwIjoxNTkyMTQxOTc3fQ");
